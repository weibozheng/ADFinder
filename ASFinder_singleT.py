...
def find_overlap(l_seq,r_seq,l_pos,r_pos,l_m,r_s,dis,l_l,l_r):
	for i in range(l_l-20,l_m,-1):
		...
		if same/max_same>0.9:
			...
			return alter_seq
	return '0'
help_str='ASFinder.py -x <genome reference in fasta> [--s1 <paired-end reads 1> --s2 <paired-end reads 2>]|[-U <Unpaired reads>]|[-S sam file] -o <output folder>'
gzinf1=''
gzinf2=''
outfile1=''
...
sam_file=''
try:
	opts,args=getopt.getopt(sys.argv[1:],"hx:p:o:U:S:",["help","s1=","s2="])
...
for opt,value in opts:
	...
	if opt in ("-S"):
		sam_file=value
if not(((gzinf1 and gzinf2) or (ref_file) or (sam_file)) and outfolder and fa_file):
	print (help_str)
	sys.exit()
else:
	try:
...
for line in inf1:
	...
	if mobj:
		...
		if mobj1:
			...
			if n1>20:
				addword2dict(dict_rec,tag_c,tag_r,line)
		if mobj2:
...
print("#3 finished")
del inf1
print("#4 Alternative splicing search part 2")
for key1 in dict_rec:
	#print key1,"\n"
	left_list_tag=list()
	right_list_tag=list()
	left_list_pos=list()
	...
	left_list_seq=list()
	right_list_seq=list()
	left_list_cigar=list()
	right_list_cigar=list()
	for key2 in dict_rec[key1]:
		if len(dict_rec[key1][key2])>=2:
			left2_list_tag=list()
			...
			for vk in dict_rec[key1][key2]:
				...
				if mobj:
					...
					if mobj1:
						SM_S=int(mobj1.group(1))
						SM_M=int(mobj1.group(2))
						right_list_tag.append(key2)
						right_list_pos.append(pos_m)
						right_list_seq.append(seq_r)
						right_list_M.append(SM_M)
						right_list_S.append(SM_S)
						right_list_cigar.append(cigar)
					if mobj2:
						MS_M=int(mobj2.group(1))
						MS_S=int(mobj2.group(2))
						left_list_tag.append(key2)
						left_list_pos.append(pos_m)
						left_list_seq.append(seq_r)
						left_list_M.append(MS_M)
						left_list_S.append(MS_S)
						left_list_cigar.append(cigar)
	for i in range(len(left_list_tag)):
		for j in range(len(right_list_tag)):
			length_read_l=left_list_M[i]+left_list_S[i]
			length_read_r=right_list_M[j]+right_list_S[j]
			dis_lr1=left_list_pos[i]+left_list_M[i]-right_list_pos[j]
			if dis_lr1>=0 and dis_lr1<=10:
				alter_seq=find_overlap(left_list_seq[i],right_list_seq[j],left_list_pos[i],right_list_pos[j],left_list_M[i],right_list_S[j],dis_lr1,length_read_l,length_read_r)
				if not alter_seq=='0':
					print ("insertion_M\t%s\t%s\t%s\t%d\t%d\t%s\t%s\t%s\t%s\t%s\t%d" % (key1,left_list_tag[i],right_list_tag[j],left_list_pos[i],right_list_pos[j],left_list_cigar[i],right_list_cigar[j],left_list_seq[i],right_list_seq[j],alter_seq,dis_lr1),file=out1e)
				else:
					print ("%d\t%d\t%s\t%s"%(left_list_pos[i]+left_list_M[i],right_list_pos[j],left_list_tag[i],right_list_tag[j]),file=out2e)
print("#4 finished")
...
num_ins=0
num_del=0
print("tag\tchromosome\tposition\tsequence\tlength\tdepth",file=out_ins)
for key1 in ins_dict:
	for key2 in ins_dict[key1]:
		if ins_dict[key1][key2]!='' and ins_num_dict[key1][key2]>=0:
			num_ins+=1
			mobj1=re.match("(\d+)_(\d+)",key2)
			print("I%d\t%s\t%s\t%s\t%d\t%d" % (num_ins,key1,mobj1.group(1),ins_dict[key1][key2],len(ins_dict[key1][key2]),ins_num_dict[key1][key2]),file=out_ins)
print("tag\tchromosome\tstart\tend\tsequence\tlength\tdepth",file=out_del)
for key1 in del_dict:
	for key2 in del_dict[key1]:
		if del_dict[key1][key2]!='' and del_num_dict[key1][key2]>=0:
			num_del+=1
			mobj2=re.match("(\d+)_(\d+)",key2)
			print("D%d\t%s\t%s\t%s\t%s\t%d\t%d" % (num_del,key1,mobj2.group(1),mobj2.group(2),del_dict[key1][key2],len(del_dict[key1][key2]),del_num_dict[key1][key2]),file=out_del)